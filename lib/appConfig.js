...
const getAppConfig = (filename = CONF) => {
  ...
  switch (os.platform()) {
    case 'win32':
      return path.resolve(homedir, 'AppData', 'Local', 'Packages', APP, filename)
    case 'darwin':
...
const randomToken = (len = 20) => crypto.randomBytes(len + 2)
  .toString('base64')
  .replace(/[+/=]/g, 'x')
  .substring(0, len)
...
class AppConfig {
  ...
  write () {
    fs.writeFileSync(this.filename, JSON.stringify(this.config, null, 2), 'utf8')
  }
  token () {
    if (!this.config) {
      this.read()
